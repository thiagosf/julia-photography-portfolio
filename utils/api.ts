const formatPhoto = (photo: any): any => {
  const link = `/photos/${photo.id}-${photo.slug}`
  const fullLink = `https://julia.photos/photos/${photo.id}-${photo.slug}`
  return {
...
export const getPhoto = async function (id: number): Promise<any> {
  const data = await fetchAPI(`
    query Photo($id: ID!) {
      photo (id: $id) {
        id
